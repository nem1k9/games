...
Unity Editor в среде разработки запустить было нельзя. Поэтому всё проверялось без него:
- **Компиляция.** Весь игровой и редакторный код собирается против настоящих сборок UnityEngine и UnityEditor (`tools/UnityCheck`, также в CI).
- **Тесты (313).** Покрыты правила, проказы, прогрессия, формат моделей, сетевой протокол и настоящий UDP (хост и два клиента, поиск игр в LAN). Для 40 сгенерированных домов проверено, что мебель не пересекается, двери свободны и мышеловки не спрятаны. Кроме того, каждый узел модели, который ищет код, есть в моделях из Blender, а каждая строка интерфейса переведена.
- **Визуальная проверка.** Уровни собираются в Blender по тем же данным и рендерятся (скриншоты выше).

**Не проверено в живой игре:** «ощущение» физики, скорости, чувствительность ИИ деда, яркость ночного освещения. Эти числа собраны в `Assets/Scripts/Core/GameConsts.cs`, и их легко подстроить после первых запусков. Для этого есть отладочные клавиши F3 и F6–F9.
