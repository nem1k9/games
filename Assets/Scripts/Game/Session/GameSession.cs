...
namespace Gnomes.Session
{
    ...
    public class PlayerSlot
    {
        ...
        public float LastCarriedByTime;
        public Color32 Color => GameSession.HatColor(Hat);
        public bool IsLocal => Peer < 0 || (GameSession.I != null && Id == GameSession.I.LocalId);
    }
    ...
    public class GameSession
    {
        ...
        public string Error;
        public bool Connected; // client: handshake done
        public float TimeLeft;
        public TaskTracker ClientTasks; // clients mirror the task list
        public int SpareHats;
        public bool AnyDead
        {
            get
            {
                foreach (var p in Players.Values) if (p.Status == PlayerStatus.Dead) return true;
                return false;
            }
        }
        ...
        uint snapSeq;
        float snapTimer;
        float timeOffset; // client: hostTime - localTime
        bool haveOffset;
        public const float InterpDelay = 0.12f;
        float connectStart;
        readonly Dictionary<int, byte> peerToPlayer = new Dictionary<int, byte>();

        static readonly Color32[] Hats =
        {
            new Color32(216, 52, 44, 255), new Color32(47, 111, 214, 255), new Color32(58, 168, 69, 255),
            new Color32(226, 178, 30, 255), new Color32(155, 68, 201, 255), new Color32(239, 122, 36, 255),
            new Color32(231, 154, 168, 255), new Color32(40, 170, 170, 255),
        };

        public static Color32 HatColor(byte i) => Hats[i % Hats.Length];
        public static int HatCount => Hats.Length;

        public PlayerSlot LocalSlot => Players.TryGetValue(LocalId, out var s) ? s : null;
        public float HostTime => IsHost ? Time.time : Time.time + timeOffset;
        ...
        {
            try { Net?.Dispose(); } catch (Exception e) { Debug.LogWarning(e); }
            Net = null;
            if (GameWorld.Current != null) UnityEngine.Object.Destroy(GameWorld.Current.gameObject);
        ...
        {
            Net?.Poll();
            if (!IsHost && !Connected && Time.time - connectStart > 12f && Error == null)
            {
            ...
            if (IsHost)
            {
                ...
                // remote gnome proxies on the host interpolate from their own state stream
                var w = GameWorld.Current;
                if (w != null)
                    foreach (var g in w.Gnomes.Values)
                        if (g is RemoteGnome rg) rg.Tick(Time.time);
            }
            else
            {
                var w = GameWorld.Current;
        ...
        {
            if (IsHost) Host.FixedUpdate(Time.fixedDeltaTime);
        }
        ...
        public void SendEventTo(PlayerSlot p, EventMsg e)
        {
            ...
            if (p.Peer < 0)
            {
                ApplyEvent(e, true); // local player on the host
                return;
            }
            writer.Reset();
            e.Write(writer);
        ...
        public void SubmitLocalState(LocalGnome g)
        {
            ...
            {
                Pos = g.transform.position.ToCoreV(),
                Vel = g.Velocity.ToCoreV(),
                Yaw = g.Yaw,
                Pitch = g.Pitch,
        ...
        void SendSnapshot()
        {
            ...
            {
                if (!full && p.Rb.IsSleeping()) continue;
                s.Props.Add(new PropPose { Id = p.Id, Pos = p.transform.position.ToCoreV(), Rot = p.transform.rotation.ToCoreQ() });
            }
        ...
        void OnData(int peer, byte[] data, int offset, int count)
        {
            ...
            {
                Debug.LogWarning($"[Net] bad {type} message: {e.Message}");
            }
        }
        ...
        void HostReceive(int peer, Msg type, NetReader r)
        {
            if (type == Msg.Hello)
            {
                ...
                // make hats unique so friends can tell each other apart
            ...
            switch (type)
            {
                case Msg.PlayerState:
                    ...
                    if (sl.Status == PlayerStatus.Free && GameWorld.Current != null && GameWorld.Current.Gnomes.TryGetValue(pid, out var g) && g is RemoteGnome rg)
                        rg.Push(Time.time, st.Pos.U(), st.Yaw, st.Pitch, st.Flags, st.Arms, st.Vel.U());
                    break;
                case Msg.Action:
        ...
        void ClientLoadLevel(LoadLevelMsg m)
        {
            ...
            foreach (var ps in m.Props) world.SpawnProp(ps.Id, ps.Kind, ps.Pos.U(), ps.Rot.U());
        ...
        /// <summary>Spawn gnome objects for all players (local one controllable).</summary>
        ...
        void ClientSnapshot(SnapshotMsg s)
        {
            ...
            foreach (var pp in s.Props)
            {
                ...
                if (p != null) p.PushPose(s.Time, pp.Pos.U(), pp.Rot.U());
            }
            foreach (var pl in s.Players)
            {
                ...
                if (g is LocalGnome lg)
                {
                    ...
                    if (pl.Status != PlayerStatus.Free) lg.ForcedPos = pl.Pos.U();
                }
                else if (g is RemoteGnome rg)
                {
                    rg.Status = pl.Status;
                    rg.Push(s.Time, pl.Pos.U(), pl.Yaw, pl.Pitch, pl.Flags, pl.Arms, Vector3.zero);
                }
            }
        ...
        public void ApplyEvent(EventMsg e, bool local)
        {
            ...
            switch (e.Type)
            {
                ...
                case EvType.PropSpawned:
                    if (!local && w != null)
                    {
                        var p = w.SpawnProp(e.Id, e.S, e.Pos.U(), e.Rot.U());
                ...
                case EvType.Sound:
                    if (!local)
                    {
                        Sfx.I?.Play((SoundId)e.P, e.Pos.U(), e.F < 0 ? 1f : e.F);
                        if (e.F < 0 && e.P == (byte)SoundId.Break) Fx.Shards(w, e.Pos.U(), "");
                    }
                    break;
                ...
                case EvType.PlayerStatus:
                    ...
                    if (w != null && w.Gnomes.TryGetValue(e.P, out var gg))
                    {
                        ...
                        if (gg is LocalGnome lg)
                        {
                            ...
                            if (e.Id == 1) lg.Teleport(e.Pos.U(), lg.Yaw); // revived / freed somewhere specific
                        }
                        else if (gg is RemoteGnome rg && e.Id == 1) rg.SnapTo(e.Pos.U());
                    }
                    break;
                case EvType.Pocket:
                ...
                case EvType.PlayerLeft:
                    Players.Remove(e.P);
                    if (w != null && w.Gnomes.TryGetValue(e.P, out var lg2))
                    {
                        w.Gnomes.Remove(e.P);
                        UnityEngine.Object.Destroy(lg2.gameObject);
                    }
                    break;
                ...
                case EvType.Banked:
                    ...
                    break;
                case EvType.Honk:
                    if (!local) Sfx.I?.Play(SoundId.Honk, e.Pos.U(), 1f);
                    break;
                case EvType.Tie:
